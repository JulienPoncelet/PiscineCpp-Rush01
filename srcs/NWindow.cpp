...
NWindow::NWindow(int const x, int const y, int const h, int const w) : _x(x), _y(y), _h(h), _w(w)
{
    WINDOW	*win;
...
void				NWindow::refreshNWindow(void) {
    refresh();
}
...
WINDOW 				*NWindow::getNWindow(void) const {
    return this->_window;
}
